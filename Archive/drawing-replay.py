...
while(not quit):
    try:
        if (state == 0):
            # CORNER DETECTION BY CHECKING FOR BLACK IN THE CORNER
            ret, image = vid.read()
            ...
            # FRAME RATE MONITORING/LIMITING, and INSTANT REPLAY FUNCTIONALITY
            ...
            # LINE DRAWING BASED ON THE FIRST BLACK PIXEL
            ret, image = vid.read()
            #image = cv2.flip(image, 1)
            image = cv2.resize(image, (DISPLAY_WIDTH, DISPLAY_HEIGHT))
            lowerLimit = np.array(color_dict_HSV.get('black')[1])
