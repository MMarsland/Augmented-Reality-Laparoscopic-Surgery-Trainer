...
def gstreamer_pipeline(capture_width=1280, capture_height=720, display_width=640, display_height=360, framerate=30, flip_method=2) :
...
while(not quit):
    ...
    # by frame
    if (state == 0):
        # FOUR SECTION SPLIT VIDEO FEED, EDGE DETECTION
        ret, image = vid.read()
        ...
        # COLORED EDGE OVERLAY ON REGULAR IMAGE
        ret, image = vid.read()
        #image = cv2.flip(image, 1)
        ...
        # ADD WARNING TEXT TO IMAGE (TEXT OVERLAY)
        ...
        # LINE DETECTION (CONTOUR DETECTION WITH FIND CONTOURS)
